...
def show_constraints_tab(username):
    ...
    table_data = []
    for i, (day, shifts) in enumerate(SHIFT_STRUCTURE.items()):
        display_day = day if i != len(SHIFT_STRUCTURE) - 1 else "ראשון (אחרון)"
        row = {"יום": display_day}
    ...
    for day in df.index:
        ...
        for i, shift in enumerate(all_shifts):
            if SHIFT_STRUCTURE.get(day.replace(" (אחרון)", "")) and shift in SHIFT_STRUCTURE[day.replace(" (אחרון)", "")]:
        ...
        for day in updated_df.index:
            for shift in all_shifts:
                if updated_df.at[day, shift] == True:
                    blocked.append((day.replace(" (אחרון)", ""), shift))
